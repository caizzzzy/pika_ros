...
class Operator:
    ...
    def __init__(self, args):
        ...
        self.sync_policies = {
            'camera_color': self.normalize_policy(self.args.cameraColorPolicy, 'nearest'),
            'camera_depth': self.normalize_policy(self.args.cameraDepthPolicy, 'nearest'),
            'camera_point_cloud': self.normalize_policy(self.args.cameraPointCloudPolicy, 'nearest'),
            'arm_joint_state': self.normalize_policy(self.args.armJointStatePolicy, 'causal'),
            'arm_end_pose': self.normalize_policy(self.args.armEndPosePolicy, 'causal'),
            'localization_pose': self.normalize_policy(self.args.localizationPosePolicy, 'causal'),
            'gripper_encoder': self.normalize_policy(self.args.gripperEncoderPolicy, 'causal'),
            'imu_9axis': self.normalize_policy(self.args.imu9AxisPolicy, 'causal'),
            'lidar_point_cloud': self.normalize_policy(self.args.lidarPointCloudPolicy, 'causal'),
            'robot_base_vel': self.normalize_policy(self.args.robotBaseVelPolicy, 'causal'),
            'lift_motor': self.normalize_policy(self.args.liftMotorPolicy, 'causal'),
        }
    ...
    def find_last_not_after_index(self, data_series: List[TimeSeries], target_time: float) -> Tuple[int, float]:
        """找到最后一个不晚于目标时间的数据索引"""
        if not data_series:
            return -1, float('inf')
        ...
        for j, time_series in enumerate(data_series):
            time_diff = target_time - time_series.time
            if time_diff < 0:
                break
            closest_index = j
            closest_diff = time_diff
    ...
    def normalize_policy(self, policy: str, default_policy: str) -> str:
        policy = (policy or default_policy).strip().lower()
        if policy in ('nearest', 'causal'):
            return policy
        print(f"Warning: Unknown policy {policy}, fallback to {default_policy}")
        return default_policy

    def find_index_by_policy(self, data_series: List[TimeSeries], target_time: float, policy: str) -> Tuple[int, float]:
        if policy == 'causal':
            return self.find_last_not_after_index(data_series, target_time)
        return self.find_closest_index(data_series, target_time)
            ...
            if frame_time is not None:
                ...
                for i, name in enumerate(self.args.cameraColorNames):
                    ...
                    if self.camera_color_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.camera_color_data_time_series[i], frame_time, self.sync_policies['camera_color'])
                ...
                for i, name in enumerate(self.args.cameraDepthNames):
                    ...
                    if self.camera_depth_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.camera_depth_data_time_series[i], frame_time, self.sync_policies['camera_depth'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.armJointStateNames):
                    ...
                    if self.arm_joint_state_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.arm_joint_state_data_time_series[i], frame_time, self.sync_policies['arm_joint_state'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.localizationPoseNames):
                    ...
                    if self.localization_pose_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.localization_pose_data_time_series[i], frame_time, self.sync_policies['localization_pose'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.cameraPointCloudNames):
                    ...
                    if self.camera_point_cloud_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.camera_point_cloud_data_time_series[i], frame_time, self.sync_policies['camera_point_cloud'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.armEndPoseNames):
                    ...
                    if self.arm_end_pose_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.arm_end_pose_data_time_series[i], frame_time, self.sync_policies['arm_end_pose'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.gripperEncoderNames):
                    ...
                    if self.gripper_encoder_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.gripper_encoder_data_time_series[i], frame_time, self.sync_policies['gripper_encoder'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.imu9AxisNames):
                    ...
                    if self.imu_9axis_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.imu_9axis_data_time_series[i], frame_time, self.sync_policies['imu_9axis'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.lidarPointCloudNames):
                    ...
                    if self.lidar_point_cloud_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.lidar_point_cloud_data_time_series[i], frame_time, self.sync_policies['lidar_point_cloud'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.robotBaseVelNames):
                    ...
                    if self.robot_base_vel_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.robot_base_vel_data_time_series[i], frame_time, self.sync_policies['robot_base_vel'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                ...
                for i, name in enumerate(self.args.liftMotorNames):
                    ...
                    if self.lift_motor_data_time_series[i]:
                        closest_idx, closest_diff = self.find_index_by_policy(
                            self.lift_motor_data_time_series[i], frame_time, self.sync_policies['lift_motor'])
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
...
def get_arguments():
    ...
    parser.add_argument('--liftMotorNames', action='store', type=str, help='liftMotorNames',
                       default=[], required=False)
    parser.add_argument('--cameraColorPolicy', action='store', type=str, help='cameraColorPolicy',
                       default='nearest', required=False)
    parser.add_argument('--cameraDepthPolicy', action='store', type=str, help='cameraDepthPolicy',
                       default='nearest', required=False)
    parser.add_argument('--cameraPointCloudPolicy', action='store', type=str, help='cameraPointCloudPolicy',
                       default='nearest', required=False)
    parser.add_argument('--armJointStatePolicy', action='store', type=str, help='armJointStatePolicy',
                       default='causal', required=False)
    parser.add_argument('--armEndPosePolicy', action='store', type=str, help='armEndPosePolicy',
                       default='causal', required=False)
    parser.add_argument('--localizationPosePolicy', action='store', type=str, help='localizationPosePolicy',
                       default='causal', required=False)
    parser.add_argument('--gripperEncoderPolicy', action='store', type=str, help='gripperEncoderPolicy',
                       default='causal', required=False)
    parser.add_argument('--imu9AxisPolicy', action='store', type=str, help='imu9AxisPolicy',
                       default='causal', required=False)
    parser.add_argument('--lidarPointCloudPolicy', action='store', type=str, help='lidarPointCloudPolicy',
                       default='causal', required=False)
    parser.add_argument('--robotBaseVelPolicy', action='store', type=str, help='robotBaseVelPolicy',
                       default='causal', required=False)
    parser.add_argument('--liftMotorPolicy', action='store', type=str, help='liftMotorPolicy',
                       default='causal', required=False)
