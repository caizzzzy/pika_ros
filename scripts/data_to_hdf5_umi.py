...
class Operator:
    def __init__(self, args):
        ...
        # 新增：相对位姿目录 + 复用绝对位姿的sync.txt
        self.localizationPoseRelToT0Dirs = [os.path.join(self.episodeDir, "localization/pose/relative_to_t0/" + self.args.localizationPoseNames[i]) for i in range(len(self.args.localizationPoseNames))]
        self.localizationPoseRelToNextDirs = [os.path.join(self.episodeDir, "localization/pose/relative_to_next/" + self.args.localizationPoseNames[i]) for i in range(len(self.args.localizationPoseNames))]
        self.localizationPoseRelSyncDirs = self.localizationPoseSyncDirs  # 核心：复用绝对位姿的sync.txt
    ...
    def process(self):
        ...
        # 新增：相对位姿字段
        for localizationPoseName in self.args.localizationPoseNames:
            data_dict[f'localization/relative_to_t0/{localizationPoseName}'] = []
            data_dict[f'localization/relative_to_next/{localizationPoseName}'] = []
        ...
        # 新增：处理relative_to_t0相对位姿（复用绝对位姿的sync.txt）
        for i in range(len(self.args.localizationPoseNames)):
            with open(self.localizationPoseRelSyncDirs[i], 'r') as lines:
                count = 0
                for line in lines:
                    line = line.replace('\n', '')
                    try:
                        with open(os.path.join(self.localizationPoseRelToT0Dirs[i], line), 'r') as file:
                            data = json.load(file)
                            # 相对位姿是x/y/z + qx/qy/qz/qw（7维）
                            data_dict[f'localization/relative_to_t0/{self.args.localizationPoseNames[i]}'].append(
                                np.array([data['x'], data['y'], data['z'], data['qx'], data['qy'], data['qz'], data['qw']], dtype=np.float64)
                            )
                    except FileNotFoundError:
                        print(f"[WARN] 跳过 {self.localizationPoseRelToT0Dirs[i]}/{line}：文件不存在")
                        data_dict[f'localization/relative_to_t0/{self.args.localizationPoseNames[i]}'].append(np.zeros(7))
                    count += 1

        # 新增：处理relative_to_next相对位姿（复用绝对位姿的sync.txt）
        for i in range(len(self.args.localizationPoseNames)):
            with open(self.localizationPoseRelSyncDirs[i], 'r') as lines:
                count = 0
                for line in lines:
                    line = line.replace('\n', '')
                    try:
                        with open(os.path.join(self.localizationPoseRelToNextDirs[i], line), 'r') as file:
                            data = json.load(file)
                            data_dict[f'localization/relative_to_next/{self.args.localizationPoseNames[i]}'].append(
                                np.array([data['x'], data['y'], data['z'], data['qx'], data['qy'], data['qz'], data['qw']], dtype=np.float64)
                            )
                    except FileNotFoundError:
                        print(f"[WARN] 跳过 {self.localizationPoseRelToNextDirs[i]}/{line}：文件不存在")
                        data_dict[f'localization/relative_to_next/{self.args.localizationPoseNames[i]}'].append(np.zeros(7))
                    count += 1
