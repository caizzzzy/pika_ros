...
def _iter_episode_dirs(root_dir: Path) -> List[Path]:
    """从根目录中筛选 episodeN 子目录（N 为数字），按数字排序返回。"""
    episodes = []
    for p in root_dir.iterdir():
        if not p.is_dir() or not p.name.startswith("episode"):
            continue
        suffix = p.name[len("episode") :]
        if suffix.isdigit():
            episodes.append((int(suffix), p))
    episodes.sort(key=lambda item: item[0])
    return [p for _, p in episodes]


def batch_process_root_dir(
    root_dir: Path,
    overwrite: bool = False,
    keep_last_identity: bool = True,
) -> None:
    """遍历根目录下的 episode 子目录并批量转换。

    - 无 localization/pose/pika 目录或有效帧数不足 2 条时跳过；
    - 单个 episode 出错不影响后续，最终给出汇总统计。
    """
    if not root_dir.exists():
        raise FileNotFoundError(f"未找到根目录: {root_dir}")

    episodes = _iter_episode_dirs(root_dir)
    print(f"[INFO] 开始批量处理根目录：{root_dir}")
    if not episodes:
        print("[WARN] 根目录下未发现 episode* 子目录，未进行任何处理。")
        return

    print(f"[INFO] 发现{len(episodes)}个episode目录：" + ", ".join(ep.name for ep in episodes))

    reason_counter = Counter()
    success_cnt = 0
    total = len(episodes)

    for idx, ep_dir in enumerate(episodes, start=1):
        print(f"[INFO] 正在处理 {ep_dir.name}（{idx}/{total}）...")
        abs_pose_dir = ep_dir / "localization" / "pose" / "pika"

        if not abs_pose_dir.exists():
            reason_counter["未找到pose目录"] += 1
            print(f"[WARN] 跳过{ep_dir.name}：未找到 localization/pose/pika 目录")
            continue

        try:
            convert_abs_to_relative(
                abs_pose_dir=abs_pose_dir,
                overwrite=overwrite,
                keep_last_identity=keep_last_identity,
            )
            success_cnt += 1
            ...
            reason_counter["有效位姿不足2条"] += 1
            print(f"[WARN] 跳过{ep_dir.name}：{exc}")
        except Exception as exc:  # noqa: BLE001
            reason_counter["处理异常"] += 1
            print(f"[ERROR] 处理 {ep_dir.name} 失败：{exc}")

    skipped = total - success_cnt
    if reason_counter:
        reason_str = "; ".join(f"{k}{v}个" for k, v in reason_counter.items())
    else:
        reason_str = "无"
    print(f"[SUMMARY] 批量处理完成：成功处理{success_cnt}个episode，跳过{skipped}个（{reason_str}）")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="根目录（包含 episodeN 子目录），例如 /home/robot/agilex/data_umi309；与 --abs-pose-dir 互斥时优先使用 abs 以兼容旧脚本。",
    )
    parser.add_argument(
        "--abs-pose-dir",
        type=Path,
...
def main():
    ...
    if args.run_self_test:
        _run_self_test()
        return
    if args.abs_pose_dir is not None:
        convert_abs_to_relative(
    ...
    raise SystemExit("请通过 --root-dir 指定包含多个 episode 的根目录，或使用 --abs-pose-dir 处理单个目录，或使用 --run-self-test 运行示例")
