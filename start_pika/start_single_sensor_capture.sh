...
SENSOR_SCRIPT="${SCRIPTS_DIR}/start_single_sensor.bash"

DATASET_DIR="${HOME}/agilex/data_single_pika"
DEFAULT_EPISODE_INDEX=0
DATA_CAPTURE_TYPE="single_pika"

LOG_ROOT="${PROJECT_ROOT}/start_pika/logs"
RUN_ID="$(date +%Y%m%d_%H%M%S)"
RUN_LOG_DIR="${LOG_ROOT}/single_sensor_${RUN_ID}"
...
prepare_sudo() {
    if ! command -v sudo >/dev/null 2>&1; then
        abort "未找到 sudo，无法执行 start_single_sensor.bash 中的设备权限配置。"
    fi

    log "即将检查 sudo 权限，这一步是启动单传感器链路所必需的。"
    if ! sudo -v; then
...
main() {
    ...
    start_managed_process \
        "single_sensor" \
        "${RUN_LOG_DIR}/single_sensor.log" \
        "${common_prefix}
${optional_conda_reset_prefix}
cd \"${SCRIPTS_DIR}\"
export PATH=/usr/bin:\$PATH
exec bash \"${SENSOR_SCRIPT}\""
