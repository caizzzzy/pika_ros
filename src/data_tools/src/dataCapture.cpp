    ...
    void armEndPoseSaving(const int index){
        while(true){
            ...
            if(rclcpp::Time(msg.header.stamp).seconds() == 0)
                break;
            Json::Value root;
            if(armEndPoseOrients.at(index)){//single_pika_teleop_data_params.yaml里面 orients: [true]，所以走这个分支
                ...
                root["pitch"] = pitch;
                root["yaw"] = yaw; 
            }else{//orients如果是false就直接保存x, y, z, w 原始值
                root["x"] = msg.pose.position.x;
                root["y"] = msg.pose.position.y;
                root["z"] = msg.pose.position.z;
                ...
                root["grasper"] = msg.pose.orientation.w;//你看到的这个脚本是一个兼容性的封装。它既要处理标准上游发来的正规四元数（需要经过 3x3 矩阵中转算 RPY），又要兼容某些不讲武德的上游发来的“伪装成四元数的 RPY + 夹爪数据”（直接提取即可）。
            }
            Json::StyledStreamWriter streamWriter;
